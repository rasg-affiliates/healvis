...
from healvis import sky_model, simulator, beam_model, utils
...
def test_run_simulation():
    ...
    # edit
    param_dict["telescope"]["array_layout"] = os.path.join(
        DATA_PATH + "/configs",
        os.path.basename(param_dict["telescope"]["array_layout"]),
    )
    param_dict["beam"]["beam_type"] = os.path.join(
        DATA_PATH, "HERA_NF_dipole_power.beamfits"
    )
...
def test_run_simulation_partial_freq():
    ...
    simulator.run_simulation_partial_freq(
        freq_chans, test_uvh5, skymod_file, fov=180, beam=beam
    )
...
def test_setup_light_uvdata():
    ...
    obs = simulator.setup_observatory_from_uvdata(
        uvd, fov=30, set_pointings=True, beam=beam_type
    )

    uvd_full = simulator.setup_uvdata(
        array_layout=os.path.join(DATA_PATH, "configs/HERA65_layout.csv"),
        telescope_location=(-30.72152777777791, 21.428305555555557, 1073.0000000093132),
        telescope_name="HERA",
        Ntimes=60,
    ...
    obs_full = simulator.setup_observatory_from_uvdata(
        uvd_full, fov=30, set_pointings=True, beam=beam_type
    )

    assert len(uvd.baseline_array) == len(bls)
    assert (
        len(uvd.time_array) == uvd.Ntimes
    )  # If the "light" weren't enabled, these would be length Nblts
...
def test_parse_freq_params():
    ...
    # case i
    fdict = simulator.parse_frequency_params(
        dict(start_freq=start_freq, Nfreqs=Nfreqs, channel_width=channel_width)
    )
    for k in master_fdict:
        assert np.all(np.isclose(master_fdict[k], fdict[k]))
    # case ii
    fdict = simulator.parse_frequency_params(
        dict(start_freq=start_freq, Nfreqs=Nfreqs, bandwidth=bandwidth)
    )
    for k in master_fdict:
        assert np.all(np.isclose(master_fdict[k], fdict[k]))
    # case iii
    fdict = simulator.parse_frequency_params(
        dict(start_freq=start_freq, Nfreqs=Nfreqs, end_freq=end_freq)
    )
    for k in master_fdict:
        assert np.all(np.isclose(master_fdict[k], fdict[k]))
    # case iv
    fdict = simulator.parse_frequency_params(
        dict(start_freq=start_freq, end_freq=end_freq, channel_width=channel_width)
    )
    for k in master_fdict:
        assert np.all(np.isclose(master_fdict[k], fdict[k]))
    ...
    simtest.assert_raises_message(
        ...
        simulator.parse_frequency_params,
        dict(freq_array=np.array([100.0])),
    )
    ...
    simtest.assert_raises_message(
        ...
        simulator.parse_frequency_params,
        dict(start_freq=100, end_freq=101, channel_width=0.33),
    )
...
def test_parse_time_params():
    ...
    time_cadence = 100.0
    Ntimes = 10
    duration = Ntimes * time_cadence / (24.0 * 3600.0)
    ...
    # now test that parsing of various key combinations all agree
    # case i
    tdict = simulator.parse_time_params(
        dict(start_time=start_time, Ntimes=Ntimes, time_cadence=time_cadence)
    )
    for k in master_tdict:
        assert np.all(np.isclose(master_tdict[k], tdict[k]))
    # case ii
    tdict = simulator.parse_time_params(
        dict(start_time=start_time, Ntimes=Ntimes, duration=duration)
    )
    for k in master_tdict:
        assert np.all(np.isclose(master_tdict[k], tdict[k]))
    # case iii
    tdict = simulator.parse_time_params(
        dict(start_time=start_time, Ntimes=Ntimes, end_time=end_time)
    )
    for k in master_tdict:
        assert np.all(np.isclose(master_tdict[k], tdict[k]))
    # case iv
    tdict = simulator.parse_time_params(
        dict(start_time=start_time, end_time=end_time, time_cadence=time_cadence)
    )
    for k in master_tdict:
        assert np.all(np.isclose(master_tdict[k], tdict[k]))
    ...
    simtest.assert_raises_message(
        ...
        simulator.parse_time_params,
        dict(time_array=np.array([100.0])),
    )
    ...
    simtest.assert_raises_message(
        ...
        simulator.parse_time_params,
        dict(start_time=100, end_time=101, time_cadence=0.33),
    )
...
def test_redundant_setup():
    ...
    time_array = np.linspace(
        start_time, start_time + Ntimes * time_cadence, Ntimes, endpoint=False
    )
    freq_array = np.linspace(
        start_freq, start_freq + Nfreqs * channel_width, Nfreqs, endpoint=False
    )

    telescope_location = (-30.72152777777791, 21.428305555555557, 1073.0000000093132)

    uvd = simulator.setup_uvdata(
        array_layout=os.path.join(DATA_PATH, "perfect_hex37_14.6m.csv"),
