...
class SkyModel(object):
    ...
    def _update(self):
        ...
        if "freqs" in ud:
            self.Z_array = f21 / self.freqs - 1.0
            self.r_mpc = comoving_distance(self.Z_array)
            self.Nfreqs = self.freqs.size
        if "Nside" in ud:
            if self.Npix is None:
                self.Npix = 12 * self.Nside ** 2
            self.pix_area_sr = 4 * np.pi / (12.0 * self.Nside ** 2)
            if "indices" not in ud and self.indices is None:
        ...
        if "data" in ud:
            ...
            s = self.data.shape
            if len(s) == 2:
                if s[0] != self.Npix or s[1] != self.Nfreqs:
    ...
    def make_flat_spectrum_shell(self, sigma, shared_memory=False):
        ...
        self.data = flat_spectrum_noise_shell(
            sigma,
            self.freqs,
            self.Nside,
            self.Nskies,
    ...
    def read_hdf5(
        ...
        with h5py.File(filename, "r") as infile:
            if do_not_overwrite_freqs:
                ...
                if self.freqs is not None and not np.allclose(
                    self.freqs, sky_freqs_part
                ):
                    raise ValueError(
                        "Currently set frequencies do not match any subset of file's frequencies."
                    )
            ...
            for sa in standard_attrs:
                if sa not in infile.attrs:
                    warnings.warn(
                        f"{sa} not in file attributes. Inferring from array shapes."
                    )
        ...
        if self.Nside is None:
            try:
                ...
                raise ValueError(
                    "Data array is not a full HEALPix map, and Nside not provided."
                )
        self._update()
    ...
    def write_hdf5(self, filename, clobber=False):
        ...
        with h5py.File(filename, "w") as fileobj:
            for k in self.valid_params:
                ...
                if k in self.dsets:
                    if np.isscalar(d):
                        fileobj.create_dataset(k, data=d, dtype=self.dsets[k])
                    else:
...
def flat_spectrum_noise_shell(
    sigma, freqs, Nside, Nskies, ref_chan=0, shared_memory=False
):
    """
...
def gsm_shell(Nside, freqs, use_2016=False):
    ...
    assert (
        pygsm_import
    ), "Couldn't import pygsm package. This is required to use GSM functionality."
    if use_2016:
        maps = pygsm.GlobalSkyModel2016(freq_unit="Hz", unit="TCMB").generate(
            freqs
        )  # Units K
    else:
        maps = pygsm.GlobalSkyModel(freq_unit="Hz", basemap="haslam").generate(
            freqs
        )  # Units K
    ...
    for fi, f in enumerate(freqs):
        maps[fi] = rot.rotate_map_pixel(
            maps[fi]
        )  # Convert to equatorial coordinates (ICRS)
